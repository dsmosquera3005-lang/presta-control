import { Y as reactExports, K as isPromise, L as isRedirect, J as isNotFound, F as invariant, h as createControlledPromise, a4 as rootRouteId, M as isServer, x as functionalUpdate, b as arraysEqual, i as createLRUCache, f as compileDecodeCharMap, a5 as trimPath, a3 as rewriteBasepath, g as composeRewrites, X as processRouteTree, W as processRouteMasks, a2 as resolvePath, d as cleanPath, a7 as trimPathRight, V as parseHref, s as executeRewriteInput, G as isDangerousProtocol, Z as redirect, w as findSingleMatch, n as deepEqual, D as DEFAULT_PROTOCOL_ALLOWLIST, c as buildRouteBranch, E as interpolatePath, U as nullReplaceEqualDeep, $ as replaceEqualDeep, Q as last, m as decodePath, u as findFlatMatch, v as findRouteMatch, C as hasKeys, t as executeRewriteOutput, p as encodePathLikeUrl, a6 as trimPathLeft, N as joinPaths, a9 as useRouter, o as dummyMatchContext, S as matchContext, z as getDefaultExportFromCjs, a0 as requireReactDom, r as exactPathTest, _ as removeTrailingSlash, R as React, P as jsxRuntimeExports, I as isModuleNotFoundError, a8 as useHydrated, q as escapeHtml, H as isInlinableStylesheet, y as getAssetCrossOrigin, a1 as resolveManifestAssetLink, O as Outlet } from "./worker-entry-DR4bSXle.js";
...
const appCss = "/assets/styles-PO9xY6KH.css";
function createSupabaseClient() {
  const SUPABASE_URL = "https://jfckpkmkxbutvbukgtce.supabase.co";
  const SUPABASE_PUBLISHABLE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpmY2twa21reGJ1dHZidWtndGNlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk3MjUzNzMsImV4cCI6MjA5NTMwMTM3M30.oSnq2jUgcH4t3ZoTGb0aMK-UTPcu-eXP94STHx3v3cY";
...
function AuthProvider({ children }) {
  ...
  const fetchRole = async (userId) => {
    ...
    const r2 = data?.role ?? null;
    setRole(r2);
    const { data: prof } = await supabase.from("profiles").select("is_active, blocked_until").eq("id", userId).maybeSingle();
    let active = prof?.is_active ?? true;
    const blockedUntil = prof?.blocked_until;
    if (!active && blockedUntil) {
      const today = localIsoDate();
      if (blockedUntil <= today) {
        await supabase.from("profiles").update({ is_active: true, blocked_until: null }).eq("id", userId);
        active = true;
      }
    }
    setIsActive(active);
    if (r2 !== "admin" && active === false) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AuthContext.Provider, { value: { session, user, role, isActive, loading, signOut, refreshRole }, children });
}
function useAuth() {
...
function playBeep() {
  try {
    ...
    g.gain.exponentialRampToValueAtTime(1e-4, ctx.currentTime + 0.3);
    o.start();
...
function ChatNotificationsProvider({ children }) {
  const { user } = useAuth();
  const [unreadByConv, setUnreadByConv] = reactExports.useState({});
  ...
  const myConvsRef = reactExports.useRef(/* @__PURE__ */ new Set());
  reactExports.useEffect(() => {
    if (!user) {
      setUnreadByConv({});
      myConvsRef.current = /* @__PURE__ */ new Set();
      return;
    }
    let cancelled = false;
    (async () => {
      const { data: parts } = await supabase.from("conversation_participants").select("conversation_id, last_read_at").eq("user_id", user.id);
      if (cancelled || !parts) return;
      myConvsRef.current = new Set(parts.map((p) => p.conversation_id));
      const counts = {};
      await Promise.all(
        parts.map(async (p) => {
          let q = supabase.from("messages").select("id", { count: "exact", head: true }).eq("conversation_id", p.conversation_id).neq("sender_id", user.id);
  ...
  reactExports.useEffect(() => {
    ...
    const channel = supabase.channel(`chat-notifications-${user.id}`).on(
      "postgres_changes",
      ...
      async (payload) => {
        ...
        const isActive = activeRef.current === m.conversation_id && typeof document !== "undefined" && document.visibilityState === "visible";
        if (isActive) {
          void supabase.from("conversation_participants").update({ last_read_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("conversation_id", m.conversation_id).eq("user_id", user.id);
          return;
        }
        ...
        const title = conv?.type === "group" ? `${senderName} en #${conv?.name ?? "Grupo"}` : senderName;
        const body = m.message_type === "text" ? m.content ?? "" : m.message_type === "image" ? "📷 Imagen" : m.message_type === "audio" ? "🎤 Audio" : "📎 Documento";
  ...
  const markRead = (conversationId) => {
    ...
    if (user) {
      void supabase.from("conversation_participants").update({ last_read_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("conversation_id", conversationId).eq("user_id", user.id);
    }
  };
  const unreadTotal = Object.values(unreadByConv).reduce((a, b) => a + b, 0);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Ctx.Provider, { value: { unreadTotal, unreadByConv, markRead, activeConvId, setActiveConvId }, children });
}
...
const Route$e = createRootRoute({
  head: () => ({
    meta: [
      ...
      { title: "PrestaControl — Gestión de Préstamos" },
      { name: "description", content: "Sistema integral para gestionar clientes, préstamos y cobros." },
      { name: "author", content: "Lovable" },
      { property: "og:title", content: "PrestaControl — Gestión de Préstamos" },
      { property: "og:description", content: "Sistema integral para gestionar clientes, préstamos y cobros." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary" },
      { name: "twitter:site", content: "@Lovable" },
      { name: "twitter:title", content: "PrestaControl — Gestión de Préstamos" },
      { name: "twitter:description", content: "Sistema integral para gestionar clientes, préstamos y cobros." },
      { property: "og:image", content: "https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/89bb8cd4-5945-4d55-82b8-107a183b2d57/id-preview-fb45d373--2b1b29d3-d6d8-40ce-b371-8a1ad4d0f8e1.lovable.app-1777875980458.png" },
      { name: "twitter:image", content: "https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/89bb8cd4-5945-4d55-82b8-107a183b2d57/id-preview-fb45d373--2b1b29d3-d6d8-40ce-b371-8a1ad4d0f8e1.lovable.app-1777875980458.png" }
    ],
    links: [
      {
        rel: "stylesheet",
...
function RootComponent() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AuthProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(ChatNotificationsProvider, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Outlet, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Toaster2, { richColors: true, position: "top-right" })
  ] }) });
}
const $$splitComponentImporter$d = () => import("./reports-D6s4Htk0.js");
const Route$d = createFileRoute("/reports")({
  component: lazyRouteComponent($$splitComponentImporter$d, "component")
});
const $$splitComponentImporter$c = () => import("./login-DfU42-Dg.js");
const Route$c = createFileRoute("/login")({
  component: lazyRouteComponent($$splitComponentImporter$c, "component")
});
const $$splitComponentImporter$b = () => import("./loans-B080MLBH.js");
const Route$b = createFileRoute("/loans")({
  component: lazyRouteComponent($$splitComponentImporter$b, "component")
});
const $$splitComponentImporter$a = () => import("./dashboard-D-ftx6nr.js");
const Route$a = createFileRoute("/dashboard")({
  component: lazyRouteComponent($$splitComponentImporter$a, "component")
});
const $$splitComponentImporter$9 = () => import("./clients-DyQvL4A0.js");
const Route$9 = createFileRoute("/clients")({
  component: lazyRouteComponent($$splitComponentImporter$9, "component")
});
const $$splitComponentImporter$8 = () => import("./chat-BIp-Zx0s.js");
const Route$8 = createFileRoute("/chat")({
  component: lazyRouteComponent($$splitComponentImporter$8, "component")
});
const $$splitComponentImporter$7 = () => import("./cash-D4lovZpb.js");
const Route$7 = createFileRoute("/cash")({
  component: lazyRouteComponent($$splitComponentImporter$7, "component")
});
const $$splitComponentImporter$6 = () => import("./index-ChsVi9m2.js");
const Route$6 = createFileRoute("/")({
  component: lazyRouteComponent($$splitComponentImporter$6, "component")
});
const $$splitComponentImporter$5 = () => import("./receipt._id-Sm8WHYxN.js");
const Route$5 = createFileRoute("/receipt/$id")({
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("./clients.new-Cdy4skIA.js");
const Route$4 = createFileRoute("/clients/new")({
  validateSearch: (search) => ({
    cedula: typeof search.cedula === "string" ? search.cedula : ""
  }),
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("./clients._id-WJ7mv_Qf.js");
const Route$3 = createFileRoute("/clients/$id")({
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("./admin.users-CfPfjdTG.js");
const Route$2 = createFileRoute("/admin/users")({
  component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
const $$splitComponentImporter$1 = () => import("./admin.novelties-BisXUXqR.js");
const Route$1 = createFileRoute("/admin/novelties")({
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("./admin.daily-movements-MZFSYYXb.js");
const Route2 = createFileRoute("/admin/daily-movements")({
...
const ClientsRoute = Route$9.update({
  id: "/clients",
  path: "/clients",
  getParentRoute: () => Route$e
});
const ChatRoute = Route$8.update({
  id: "/chat",
  path: "/chat",
  getParentRoute: () => Route$e
});
const CashRoute = Route$7.update({
  id: "/cash",
  path: "/cash",
  getParentRoute: () => Route$e
});
...
const ReceiptIdRoute = Route$5.update({
  id: "/receipt/$id",
  path: "/receipt/$id",
  getParentRoute: () => Route$e
});
const ClientsNewRoute = Route$4.update({
  id: "/new",
  path: "/new",
  getParentRoute: () => ClientsRoute
});
const ClientsIdRoute = Route$3.update({
  id: "/$id",
  path: "/$id",
  getParentRoute: () => ClientsRoute
});
...
const AdminNoveltiesRoute = Route$1.update({
  id: "/admin/novelties",
  path: "/admin/novelties",
  getParentRoute: () => Route$e
});
const AdminDailyMovementsRoute = Route2.update({
  id: "/admin/daily-movements",
  path: "/admin/daily-movements",
  getParentRoute: () => Route$e
});
const ClientsRouteChildren = {
  ClientsIdRoute,
  ClientsNewRoute
};
const ClientsRouteWithChildren = ClientsRoute._addFileChildren(ClientsRouteChildren);
