import { c as createServerRpc } from "./createServerRpc-BrZOXQSF.js";
import { l as createServerFn } from "./worker-entry-DR4bSXle.js";
import { r as requireSupabaseAuth, n as normalizePhoneNumber, b as buildWhatsAppMessage } from "./whatsappHelpers-EuXWngwu.js";
import "node:events";
...
const sendWhatsAppReceipt = createServerFn({
  ...
  const {
    data: payment,
    error: paymentErr
  } = await supabase.from("payments").select("id, amount, payment_type, client_id, notes").eq("id", paymentId).maybeSingle();
  if (paymentErr) {
